...
data.mm.id=data1%>%
  ...
  select(COMPID)%>%
  left_join(data5%>%
              filter(MENGTYPE=="高血压病"|MENGTYPE=="普通门诊"),by="COMPID")
...
table.tt.3=read.csv("门慢患者他汀类开药次数.csv")
myLabel=as.vector(table.tt.3$开药次数1)
myLabel = paste(table.tt.3$人数1,"(", round(table.tt.3$人数1 / sum(table.tt.3$人数1) * 100, 2), "%", ")",sep = "")   


ggplot(table.tt.3,aes(reorder(开药次数1,-人数1),y=人数1))+
  geom_bar(stat="identity",position="dodge")+
  theme(axis.ticks.length=unit(0.5,'cm'))+
  theme(axis.title = element_blank(),legend.position='none')+ 
...
hh3=data.frame(x=c("他汀类开药人数","其他人数"),y=c(94042,414360),z=c(round(94042/508402,4),round(414360/508402,4)))
myLabel=as.vector(hh3$x)
myLabel = paste( round(hh3$y / sum(hh3$y) * 100, 2), "%", sep = "")   

hh4=data.frame(x=c("他汀类开药人数","其他人数"),y=c(133782,612555),z=c(round(133782/746337,4),round(612555/746337,4)))
...
ggplot(hh3,aes(x='',y=y,fill=x))+
  ...
  geom_text(aes(y = hh3$y/2 + c(0, cumsum(hh3$y)[-length(hh3$y)]), x = sum(hh3$y)/600000, label = myLabel))
...
library(data.table)
gxy.yy=fread("高血压患者(普通门诊+住院+门慢)用药情况.csv")
...
#.......................同时在门慢和普通门诊就诊的处方情况
gxy.1=read.csv("高血压门慢患者门慢与普通门诊的处方情况比较.csv")


ggplot(gxy.1,aes(reorder(药品类别,开药总次数),y=开药总次数,fill=MENGTYPE))+
  geom_bar(stat="identity",position="dodge")+
  theme(axis.ticks.length=unit(0.5,'cm'))+
  ggtitle("高血压门慢患者门慢与普通门诊处方情况比较")+
  guides(fill=guide_legend(title="MENGTYPE"))+
...
gxy.jz=fread("gxy.csv")

#高血压患者费用计算
gxy.fy=gxy.jz%>%
  filter(MENGTYPE=="高血压病"|MENGTYPE=="普通门诊")%>%
  group_by(COMPID)%>%
  summarise(人年均费用=sum(MEDAMOU))

mean(gxy.fy$人年均费用)    #5915.323
gxy.fy$z得分=(gxy.fy$人年均费用-mean1)/sd1

sd1=1526.83
mean1=1670.977
quantile(gxy.fy$人年均费用,0.75)-quantile(gxy.fy$人年均费用,0.25)
...
gxy.mm.fy=gxy.jz%>%
  ...
  left_join(gxy.jz%>%
              filter(MENGTYPE=="高血压病"|MENGTYPE=="普通门诊"),by="COMPID")%>%
  group_by(COMPID)%>%
  summarise(人均年费用=sum(MEDAMOU))


mean(gxy.mm.fy$人均年费用)
